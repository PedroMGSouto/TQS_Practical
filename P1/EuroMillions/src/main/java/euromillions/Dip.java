 ...
 */
public class Dip {
    private static final int const_NUMBERS = 5;
    private static final int const_MAX_NUM = 50;
    private static final int const_STARS = 2;
    private static final int const_MAX_STAR = 10;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (const_NUMBERS == arrayOfNumbers.length && const_STARS == arrayOfStarts.length) {
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < const_NUMBERS; ) {
            int candidate = generator.nextInt(const_MAX_NUM-1) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                i++;
            }
        }
        for (int i = 0; i < const_STARS; ) {
            int candidate = generator.nextInt(const_MAX_STAR-1) + 1;
